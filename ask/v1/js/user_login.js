function dologin_index() {
    var longinError_index = function() {
        $('#tlogin-info').html('错误提示：用户名或密码错误！').show();
    };
    var html = '';
    ...
    jQuery.getJSON("http://home.9939.com/buluo/index/do/login/?username=" + username + "&password=" + password + "&format=json&jsoncallback=?", function(data) {
        if (data != 'type0') {
            o = eval("(" + data + ")");
            html_index_news = '<span class="gren"><a href="http://home.9939.com/user/index/" target="_blank">' + o.username + '</a></span><span><a href="javascript:;" onclick="logout();">退出</a></span>';
...
function dologin_detail(){
    var longinError_index = function() {
        $('#answer_need_login #tlogin-info').html('错误提示：用户名或密码错误！').show();
    };
    var html = '';
    var username = $('#answer_need_login #l-name').val();
    var password = $('#answer_need_login #l-psw').val();
    if (username.length == 0 || password.length == 0) {
        longinError_index();
        return false;
    }
...
function logout() {
    jQuery.getJSON("http://home.9939.com/buluo/index/do/logout/?format=json&jsoncallback=?", function(data) {
        ...
        $('#login').html(html);
        var html_index_new = '<span class="rIcon1" id="spShow"><i></i><a href="javascript:" onclick="return test();" title="登录">登录</a></span><span class="rIcon2"><i></i><a href="http://www.9939.com/register/" target="_blank" title="注册">注册</a></span>';
