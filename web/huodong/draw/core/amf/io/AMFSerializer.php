   ...
   var $outBuffer;
	...
	function serialize(&$amfout) {
		...
		for ($i = 0; $i < $count; $i++) {
			...
			$tempBuf = $this->outBuffer;
			$this->outBuffer = "";
			$this->writeData($body->getResults()); // write the data to the output stream
			$tempBuf2 = $this->outBuffer;
			$this->outBuffer = $tempBuf;
			$this->writeLong(strlen($tempBuf2));
			$this->outBuffer .= $tempBuf2;
	...
	function writeRecordSet(&$rs) 
	{
		...
		{
			//AMF3 mode
			$mode = $GLOBALS['amfphp']['amf3RecordSetFormat'];
			
			$numObjects= 0;
			if($mode == 'ArrayCollection')
			{
				//Add the codes for an array collection
				//object_code . object_type . str_len
				$this->writeByte(0x0a);
				$this->writeByte(0x07);
			...
			if($numRows > 0)
			{
				...
				foreach ($rows as $key => $line) {
					...
					{
						//Write the col name
						$this->writeAmf3String($rs->columns[$i]);
						$value = $line[$i];
						if(is_string($value))
						{
							$os = $this->charsetHandler->transliterate($value);
							$this->writeByte(0x06);
							$this->writeAmf3String($os);
						}
						elseif(is_int($value)) 
						{ //int
	 ...
	 */
	function writeData($d, $type = -1, $subtype = -1) {
		if ($type == -1) 
		{
	...
	function writeAmf3Int($d)
	{
		...
		elseif($d < 0x4000)
		{
			$this->writeByte($d >> 7 & 0x7f | 0x80);
			$this->writeByte($d & 0x7f);
		}
		elseif($d < 0x200000)
		{
			$this->writeByte($d >> 14 & 0x7f | 0x80);
			$this->writeByte($d >> 7 & 0x7f | 0x80);
			$this->writeByte($d & 0x7f);
		} 
		else
		{
			$this->WriteByte($d >> 22 & 0x7f | 0x80);
			$this->writeByte($d >> 15 & 0x7f | 0x80);
			$this->writeByte($d >> 8 & 0x7f | 0x80);
			$this->writeByte($d & 0xff);
		}
	}

	function writeAmf3String($d)
	{
		if( $d == "" )
		{
		...
		{
			if( ($key = patched_array_search($d, $this->storedStrings, TRUE)) === FALSE)
			{
				$this->storedStrings[] = $d;
	...
	function writeAmf3Array($d)
	{
		...
		if( ($key = patched_array_search($d, $this->storedObjects, TRUE)) === FALSE )
		{
			...
			if ($str_count > 0 && $num_count == 0) { // this is a mixed array
				$this->writeAmf3ObjectFromArray($string); // write the numeric and string keys in the mixed array
	...
	function writeAmf3ObjectFromArray($d)
	{
		...
		$this->writeByte(0x0A);
		$this->writeByte(0x0B);
		$this->writeAmf3String(""); //Anonymous object
