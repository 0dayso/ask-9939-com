...
function authenticationFilter (&$amf) {
	...
	$sessId = NULL;
	if(!isset($_GET[$sessionName]) && !isset($_COOKIE[$sessionName]))
	{
		//Then we are missing the session
		//Look at the first body
		$body = $amf->getBodyAt(0);
		$target = $body->targetURI;
		$value = $body->getValue();
		
		if($target == 'null' && isset($value[0]['clientId']))
		{
			//Use the client id as the session id
			$sessId = md5($value[0]['clientId']);
		}
	}
	...
	if(!strpos($_SERVER['QUERY_STRING'], $session_id) !== FALSE)
	{
		...
		*/
		$outHeader = new MessageHeader("AppendToGatewayUrl", false, "?" . ini_get('session.name') . "=" . $session_id);
		$amf->addOutgoingHeader($outHeader);
	}
}
